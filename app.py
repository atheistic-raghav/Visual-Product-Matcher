...
# Limit TensorFlow threading and logging to reduce memory and CPU usage
...
def find_similar_products(query_embedding, top_k=12):
    ...
    try:
        ...
        results = []
        for idx in top_idxs:
            sim = max(0.0, min(1.0, similarities[idx]))
            results.append({
...
def search():
    try:
        if product_embeddings is None:
            return jsonify(success=False, message="Embeddings not loaded, please run the feature extractor.")
        ...
        results = find_similar_products(embedding)
        if not results:
...
# Load embeddings and model on import (for Gunicorn)
